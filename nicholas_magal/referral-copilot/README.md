...
- **Analytics** -- Databricks SQL query execution for referral summary and search
- **Lakebase** -- Fully managed Postgres database for transactional (OLTP) workloads on Databricks
- **Model Serving** -- Databricks-hosted LLM calls for chat and handoff brief generation
...
The chat backend uses Databricks Model Serving through AppKit. The bundle is configured to query the Databricks-hosted `databricks-meta-llama-3-1-8b-instruct` foundation-model endpoint, so no paid external LLM key is required.
